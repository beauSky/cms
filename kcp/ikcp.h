...
struct IKCPCB
{
	...
	sockaddr_in maddr;
	//snd_una 发送缓存的第一个包序号
	//snd_nxt 发送缓存的最后一个包序号
	//rcv_nxt 下一个待放入接收队列的包序号
	//rmt_wnd 对端接收窗口
	//snd_wnd, rcv_wnd 本端发送和接收窗口
	IUINT32 conv, mtu, mss, state;
	IUINT32 snd_una, snd_nxt, rcv_nxt; //snd_una 最旧未确认的序列号 snd_nxt将要发送的序列号 rcv_nxt期望接收序列号
	IUINT32 ts_recent, ts_lastack, ssthresh;
	IINT32 rx_rttval, rx_srtt, rx_rto, rx_minrto; //rto计算
	IUINT32 snd_wnd, rcv_wnd, rmt_wnd, cwnd, probe;//snd_wnd, rcv_wnd 本端发送和接收窗口 rmt_wnd 对端接收窗口 cwnd拥塞算法窗口 probe定时查询对端窗口
	IUINT32 current, interval, ts_flush, xmit;
	IUINT32 nrcv_buf, nsnd_buf;
	IUINT32 nrcv_que, nsnd_que;
	IUINT32 nodelay, updated;
	IUINT32 ts_probe, probe_wait;
	IUINT32 dead_link, incr;//incr 用于控制流量，最大待确认的字节数
	...
	char *buffer;
	int fastresend;
	int nocwnd, stream;	 //nocwnd 拥塞算法开关，0打开，>0关闭
	int logmask;
